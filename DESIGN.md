# Lojinha da Lari - Documento de Design e Decisões

Este documento contém o escopo inicial validado antes da construção da aplicação, bem como o registro de decisões tomadas durante a fase de concepção da Lojinha da Lari.

## 1. Resumo do Projeto (Understanding Summary)

*   **O que será construído:** Um site de catálogo virtual focado em performance mobile para listar os produtos da Lojinha da Lari, exibindo fotos, descrições e preços.
*   **Por que existe:** Para profissionalizar e facilitar a divulgação dos produtos, funcionando como uma vitrine digital.
*   **Para quem:** Inicialmente focado na rede local de contatos e clientes frequentes da Lari.
...
1.  **Gestão Própria:** A Lari será a responsável por cadastrar os novos produtos via celular ou desktop.
...
*   **Por que esta opção foi escolhida (Next.js + Supabase):** Combinação ideal para hospedar nativamente de forma gratuita e escalável na Vercel. Next.js fornece a melhor experiência de leitura SEO-friendly e alta velocidade de entrega para o catálogo. O Supabase consolida o banco de dados (PostgreSQL), Auth (login da Lari) e Armazenamento (Storage das fotos) em um único ecossistema, tudo dentro de free tiers conhecidos e já utilizados pelo proprietário.
...
*   **Por qual motivo:** Apesar do esforço inicial de codificar formulários e uploads serem levemente maiores, o painel feito sob medida será exatamente adaptado à necessidade da loja. Menos telas complexas significará menos tempo que a Lari perderá tentando entender botões desnecessários na hora de colocar um produto novo à venda no catálogo.
