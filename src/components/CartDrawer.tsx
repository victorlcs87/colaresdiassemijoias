...
export function CartDrawer() {
    ...
    const handleCheckout = () => {
        ...
        // Pre-gerar um template simpático e escaneável para a Lari
        const message = `Olá Lari! Peguei esses itens no catálogo:\n\n${cartDetails}\n\n🛒 *TOTAL: R$ ${total}*\n\nPode me confirmar o valor do frete e enviar a chave do Pix? 😊`;

        const encodedMessage = encodeURIComponent(message);

        // Disparo de Evento Analytics (Será capturado se o Vercel Analytics estiver injetado)
        if (typeof window !== "undefined" && (window as any).va) {
            (window as any).va('event', 'Purchase_Intent_WhatsApp', { total_value: total, total_items: getTotalItems() });
        }
    ...
    return (
        <>
            ...
            <button
                onClick={openCart}
                className="relative flex items-center justify-center w-10 h-10 rounded-full bg-white dark:bg-[#1e362a] hover:bg-slate-100 dark:hover:bg-[#2a4538] transition-colors shadow-sm text-slate-900 dark:text-slate-100 mr-2"
            >
                <ShoppingCart className="h-6 w-6" />
                {getTotalItems() > 0 && (
                    <span className="absolute -top-1 -right-1 bg-primary text-slate-900 text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-white dark:border-[#152a20] animate-in zoom-in">
                        {getTotalItems()}
                    </span>
                )}
            </button>
            ...
            {typeof document !== "undefined" && createPortal(
                <div className="cart-drawer-portal">
                    ...
                    {/* Drawer */}
                    <div className={`fixed top-0 right-0 z-[9991] h-[100dvh] w-full max-w-md bg-white dark:bg-[#152a20] shadow-2xl transition-transform duration-500 ease-out transform ${isOpen ? "translate-x-0" : "translate-x-full"} flex flex-col`}>
                        {/* Header */}
                        <div className="p-6 border-b border-slate-100 dark:border-[#2a4538] flex items-center justify-between">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {items.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                                    <div className="p-6 bg-[#f8fcfa] dark:bg-[#1b2f24] rounded-full">
                                    ...
                                    <button
                                        onClick={closeCart}
                                        className="bg-primary text-slate-900 font-bold px-8 py-3 rounded-full hover:brightness-105 transition-all shadow-sm"
                                    >
                                        Ver Catálogo
                                    </button>
                                </div>
                                ...
                                items.map((item) => (
                                    <div key={item.id} className="flex gap-4 group">
                                        <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden flex-shrink-0">
                                            <img
                                                src={item.image_url || "https://images.unsplash.com/photo-1515562141207-7a1891ce3a8e"}
                                                alt={item.name}
                        ...
                        {/* Footer */}
                        {items.length > 0 && (
                            <div className="p-6 border-t border-slate-100 dark:border-[#2a4538] bg-slate-50/50 dark:bg-[#152a20]/50 space-y-4">
                                ...
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg flex items-start gap-3">
                                    ...
                                    <p className="text-[11px] text-blue-700 dark:text-blue-300 leading-tight">
                                        Ao finalizar, você será redirecionada para o **WhatsApp da Lari** para combinar o pagamento e a entrega.
                                    </p>
                                </div>
                                <button
